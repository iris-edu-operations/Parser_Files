...
#       IP           DateTime             Bytes  UserAgent
#       --           --------             -----  ---------
# 177.20.130.209, 2016-08-23T20:00,           0, SOD/3.2.6
#  129.71.50.235, 2016-08-23T20:00,         512, Swarm/2.5.9
#  136.167.12.22, 2016-08-23T20:00,           0, IRIS-WS-Library/2.0.12
#  81.149.172.17, 2016-08-23T20:00,           0, ObsPy
...
if ( scalar @tokens != 4 ) {
	...
	die "\nExpected CSV input lines with 4 tokens 'IP, DateTime, Bytes, UserAgent' but got line line [$line]\n\n";
}
...
# - Remove spaces.
# - Replace no IP or no User agent with place holders.
my $prep_file_cmd = " cat $data_file_path | grep -v \"^Host\" $devel_head_cmd | sed 's/ //g' | awk -F \",\" '(NF == 4) {print ((\$1 == \"\") ? \"NO-IP\" : \$1),\$2,\$3,((\$4 == \"\") ? \"NO-USER-AGENT\" : \$4) }' ";
...
my @titles = (
	...
	"Highest No-data Requests by IP-Address" ,
